...
namespace ROMVault2.RvDB
{
    public class RvFile : RvBase
    {
        ...
        {
            if (!FileStatusIs(FileStatus.SizeFromHeader) && !FileStatusIs(FileStatus.SizeVerified))
            {
                Size = null;
            }
            if (!FileStatusIs(FileStatus.CRCFromHeader) && !FileStatusIs(FileStatus.CRCVerified))
            {
                CRC = null;
            }
            if (!FileStatusIs(FileStatus.SHA1FromHeader) && !FileStatusIs(FileStatus.SHA1Verified))
            {
                SHA1 = null;
            }
            if (!FileStatusIs(FileStatus.MD5FromHeader) && !FileStatusIs(FileStatus.MD5Verified))
            {
                MD5 = null;
            }
            if (!FileStatusIs(FileStatus.SHA1CHDFromHeader) && !FileStatusIs(FileStatus.SHA1CHDVerified))
            {
                SHA1CHD = null;
            }
            if (!FileStatusIs(FileStatus.MD5CHDFromHeader) && !FileStatusIs(FileStatus.MD5CHDVerified))
            {
                MD5CHD = null;
            }
        ...
        public override EFile FileRemove()
        {
            ...
            if (!FileStatusIs(FileStatus.SHA1CHDFromDAT))
            {
                SHA1CHD = null;
            }
            if (!FileStatusIs(FileStatus.MD5CHDFromDAT))
            {
                MD5CHD = null;
            }
